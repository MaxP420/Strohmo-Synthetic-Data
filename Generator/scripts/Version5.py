...
for cone in cones:
    for i in range(5):
        dub = cone["obj"].duplicate()
        dub.set_cp("category_id", cone["category_id"])
        dub.set_cp("cone_color", cone["cone_color"])
        dub.set_cp("supercategory", cone["supercategory"])
        dub.set_name(cone['name'])
        #dub.set_name(f"{cone['name']}_{i}")
        #dub.set_name(f"{cone['name']}")
